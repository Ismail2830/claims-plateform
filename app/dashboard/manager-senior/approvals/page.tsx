...
export default function ManagerSeniorApprovalsPage() {
  const router = useRouter();
  const { user, isLoading: authLoading, logout, token } = useAdminAuth();

  const { data, isLoading, mutate } = useSWR(
    token ? [`/api/manager-senior/claims?status=IN_DECISION&limit=50`, token] : null,
    ([url, t]: [string, string]) =>
    ...
    <RoleBasedLayout role="MANAGER_SENIOR" user={user} onLogout={logout}>
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-gray-900">File d&apos;approbation</h2>
            <p className="text-sm text-gray-500 mt-0.5">Dossiers en attente de décision finale</p>
          </div>
                ...
                {claims.map((c: {
                  ...
                  <tr key={c.claimId} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3">
                      <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${c.priority === 'CRITICAL' ? 'bg-red-100 text-red-700' : c.priority === 'HIGH' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}`}>{c.priority}</span>
                    </td>
                    <td className="px-5 py-3 text-gray-600">{c.estimatedAmount ? `${parseFloat(c.estimatedAmount).toLocaleString('fr-MA')} MAD` : '—'}</td>
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-1.5">
                        <button className="rounded-md bg-green-50 px-3 py-1 text-xs font-medium text-green-700 hover:bg-green-100 transition-colors">Approuver</button>
                        <button className="rounded-md bg-red-50 px-3 py-1 text-xs font-medium text-red-700 hover:bg-red-100 transition-colors">Rejeter</button>
                        <button className="rounded-md bg-yellow-50 px-3 py-1 text-xs font-medium text-yellow-700 hover:bg-yellow-100 transition-colors">Escalader</button>
                      </div>
                    </td>
                  </tr>
                ))}
