...
export default function ManagerSeniorClaimsPage() {
  ...
  const apiUrl = token
    ? `/api/manager-senior/claims?page=${page}&limit=20${statusFilter ? `&status=${statusFilter}` : ''}`
    : null;
      ...
      <div className="space-y-5">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex-1 min-w-48 flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2">
            ...
            <input placeholder="Rechercher…" className="flex-1 text-sm outline-none bg-transparent text-gray-700 placeholder:text-gray-400" />
          </div>
          <div className="flex items-center gap-2">
            <Filter className="h-4 w-4 text-gray-400" />
            <select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }} className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm text-gray-700 outline-none">
        ...
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
          {error ? (
            <div className="p-8 text-center text-sm text-red-600">Erreur de chargement.</div>
          ) : (
            <table className="w-full text-sm">
              <thead className="border-b border-gray-200 bg-gray-50">
                <tr>
                  ...
                  <th className="px-5 py-3 text-left font-semibold text-gray-600">Montant</th>
        ...
        {!isLoading && meta.pages > 1 && (
          <div className="flex items-center justify-between text-sm text-gray-600">
            <span>{meta.total} dossier(s)</span>
            <div className="flex items-center gap-2">
              <button disabled={page <= 1} onClick={() => setPage((p) => p - 1)} className="rounded-lg border border-gray-200 px-3 py-1.5 hover:bg-gray-50 disabled:opacity-40 transition-colors">Précédent</button>
              <span>Page {page} / {meta.pages}</span>
              <button disabled={page >= meta.pages} onClick={() => setPage((p) => p + 1)} className="rounded-lg border border-gray-200 px-3 py-1.5 hover:bg-gray-50 disabled:opacity-40 transition-colors">Suivant</button>
            </div>
          </div>
        )}
      </div>
